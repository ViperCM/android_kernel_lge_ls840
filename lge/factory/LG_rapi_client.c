...
#ifdef CONFIG_LGE_DIAG_ICD
int icd_send_to_arm9(void*	pReq, void* pRsp, unsigned int output_length)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	int result;
	...
	result = oem_rapi_client_streaming_function(client, &arg, &ret);
	if(result < 0)
	{
		printk(KERN_ERR "%s(), time out oem_rapi_client_streaming_function\n",__func__);
		return -1;
	}
	else
	{
		...
		{
			printk(KERN_INFO "%s(), ret.output[%d]:0x%x \n",__func__, i,ret.output[i]);
		}		
#endif
		memcpy(pRsp,ret.output,output_length);
		return 1;		
	}
}
...
//	acc_cable_type usb_cable_type = NO_INIT_CABLE;
	int errCount=0;
	int rc= -1;

// LGE_CHANGE [jaeho.cho@lge.com] 2012-02-11, workaround rpc_rsp.data.acc_hdr.accept_stat -4 err [START]
	byte temp = 0;
// LGE_CHANGE [jaeho.cho@lge.com] 2012-02-11, workaround rpc_rsp.data.acc_hdr.accept_stat -4 err [END]
...
//		if(usb_cable_type == LT_CABLE_56K || usb_cable_type == LT_CABLE_130K || usb_cable_type == LT_CABLE_910K)
//			return 0;
//		else
//			return 1;
//	}

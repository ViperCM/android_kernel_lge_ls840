...
static ssize_t result_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	len = (strlen(buf) >= sizeof(process_status)) ? sizeof(process_status) : strlen(buf);
	strncpy(process_status, buf, len);
	process_status[len] = '\0';
...
static ssize_t return_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("\n%s:%s\n", __func__,buf);
        ...
        len = (strlen(buf) >= sizeof(process_value)) ? sizeof(process_value) : strlen(buf);
        strncpy(process_value, buf,len);
	    process_value[len] = '\0';

        return count;
}
static char nfc_result_str[100];	// chan2.kim add for LG Service Center
...
static DEVICE_ATTR(result, S_IRUGO | S_IWUSR | S_IWGRP | S_IROTH, NULL, result_store);
static DEVICE_ATTR(return, S_IRUGO | S_IWUSR | S_IWGRP | S_IROTH, NULL, return_store);

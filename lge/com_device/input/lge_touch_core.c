					...
					TOUCH_INFO_MSG("<%d> pos[%4d,%4d] w_m[%2d] w_n[%2d] w_o[%2d] p[%3d]\n",
						...
						if(key_touch_logging_status == 1)
						{
							printk(KERN_INFO "%s(), down x:%d, y:%d\n",__func__, 
								ts->ts_data.curr_data[ts->ts_data.total_num].x_position, 
								ts->ts_data.curr_data[ts->ts_data.total_num].y_position );
							slate_send_touch_log_packet(ts->ts_data.curr_data[ts->ts_data.total_num].x_position, 
								ts->ts_data.curr_data[ts->ts_data.total_num].y_position, 1);
						}
...
//LGE_CHANGE_S:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
#if defined(CONFIG_MACH_LGE_C1_BOARD_SPR)
	max_x = ts->pdata->caps->x_max;
	max_y = ts->pdata->caps->y_max;
#endif	
//LGE_CHANGE_E:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>	
	return 0;

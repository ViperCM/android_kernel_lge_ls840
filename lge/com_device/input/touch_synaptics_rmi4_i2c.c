...
extern void slate_send_touch_log_packet(unsigned long touch_x, unsigned long touch_y, unsigned long status);
#endif
...
//LGE_CHANGE_S:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
					...
					{
						printk(KERN_INFO "%s(), down x:%d, y:%d\n",__func__, curr_ts_data.pos_x[f_counter], curr_ts_data.pos_y[f_counter] );						
						slate_send_touch_log_packet(curr_ts_data.pos_x[f_counter], curr_ts_data.pos_y[f_counter], 1);
					}
#endif					
//LGE_CHANGE_E:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
					report_enable = 1;
...
//LGE_CHANGE_S:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
					...
					if(key_touch_logging_status == 1)
					{
						printk(KERN_INFO "%s(),up x:%d, y:%d\n",__func__, ts->pre_ts_data.pos_x[f_counter], ts->pre_ts_data.pos_y[f_counter] );
						slate_send_touch_log_packet(ts->pre_ts_data.pos_x[f_counter], ts->pre_ts_data.pos_y[f_counter], 0);
					}
#endif					
//LGE_CHANGE_E:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
					report_enable = 1;
...
//LGE_CHANGE_S:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>
#if defined(CONFIG_MACH_LGE_C1_BOARD_SPR)
	max_x = ts->pdata->x_max;
	max_y = ts->pdata->y_max;
#endif	
//LGE_CHANGE_E:<sinjo.mattappallil@lge.com><03/06/2012><Slate feature porting from LS840 GB>

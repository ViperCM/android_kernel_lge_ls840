...
int get_ic_info(struct synaptics_ts_data* ts, struct touch_fw_info* fw_info)
{
	...
	#elif defined(CONFIG_MACH_LGE_I_BOARD_VZW) || defined(CONFIG_MACH_LGE_C1_BOARD_MPS) || defined(CONFIG_MACH_LGE_C1_BOARD_SPR) //cayman USA version
			strncpy(fw_info->fw_image_product_id, &SynaFirmware[0][16], 10);
			fw_info->fw_start = (unsigned char *)&SynaFirmware[0][0];
	#else
...
#if defined(CONFIG_MACH_LGE_I_BOARD_VZW) || defined(CONFIG_MACH_LGE_C1_BOARD_MPS) || defined(CONFIG_MACH_LGE_C1_BOARD_SPR) //cayman USA version

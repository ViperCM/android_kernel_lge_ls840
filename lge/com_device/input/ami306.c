...
static int chipset_initialized = 0;

#if defined(CONFIG_MACH_LGE_I_BOARD_VZW) || defined (CONFIG_MACH_LGE_C1_BOARD_SPR)
//seungkwan.jung
...
static int AMI306_SetMode(int newmode)
{
	...
	chipset_initialized = AMI306_Chipset_Init(newmode, chipset);

	return chipset_initialized;
}
...
//	udelay(20);
    ...
    {    
		...
		chipset_initialized = AMI306_Chipset_Init(AMI306_FORCE_MODE,ami306_data.chipset);
		return -3;
    }		
exit_AMI306_ReadSensorData:
	if (res <= 0) {
...
static ssize_t store_enable_value(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		/* if abnormaly failed to initialize the chipset while device probing(boot-up),
			check and re-initialize */
		if(chipset_initialized <= 0) {
			pdata->power_on(1<<SENSOR_TYPE_DCOMPASS);
			udelay(1000); // mdelay(1)

			chipset_initialized = AMI306_Chipset_Init(AMI306_FORCE_MODE,ami306_data.chipset);
			dev_info(dev, "%s: AMI306_Chipset init ret =%d", __func__, chipset_initialized);
		}
...
static long ami306_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd) {
		case AMI306_IOCTL_INIT:
			...
			chipset = ami306_data.chipset;
			read_unlock(&ami306_data.lock);
			chipset_initialized = AMI306_Chipset_Init(mode, chipset);
			break;
		...
		const struct i2c_device_id * devid)
{
	...
	chipset_initialized = AMI306_Chipset_Init(AMI306_FORCE_MODE, ami306_data.chipset); // default is Force State
...
static int ami306_resume(struct device *device)
{
	...
	udelay(1000);//mdelay(1);
	ami306mid_data.controldata[AMI306_CB_ACTIVESENSORS] |= (AMIT_BIT_MAGNETIC_FIELD|AMIT_BIT_ACCELEROMETER);
	chipset_initialized = AMI306_Chipset_Init(ami306_data.mode, ami306_data.chipset);

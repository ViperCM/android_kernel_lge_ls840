...
#define MAX_LEVEL_BL			0x81
#else
#define MAX_LEVEL_BL			0x71
#endif

#define MIN_LEVEL 		0x00
#define DEFAULT_LEVEL	55 //kurva.venumadhav@lge.com default brightness is changed as GB
...
static void lm3537_set_main_current_level(struct i2c_client *client, int level)
{
	...
	// printk("[Krishna]level  : %d,max_brightness :%d\n",level , max_brightness);
...
{
	lm3537_write_reg(client, 0x10, 0x00);	
}
...
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB><END>
	...
	mdelay(1);
#ifdef LOG_SCALE_MAPPING
	lm3537_write_reg(main_lm3537_dev->client, 0x30, 0x09);
#else	
	lm3537_write_reg(main_lm3537_dev->client, 0x30, 0x1B);   //speed -0x1B 0x24 0x2D 0x36
#endif	
	mdelay(1);
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB,Sensor chip is driving the 1.8V from the Backlight><START>
	lm3537_write_reg(main_lm3537_dev->client, 0xc1, 0x0c);	 //LDO1 VOUT
	mdelay(1);
	lm3537_write_reg(main_lm3537_dev->client, 0xC0, 0x01);	 //LDO ENABLE
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB,Sensor chip is driving the 1.8V from the Backlight><END>
}
...
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB><START>
//This code makes BL chip OFF,so sensor is not getting the 1.8V
#ifndef CONFIG_MACH_LGE_C1_BOARD_SPR
	gpio_tlmm_config(GPIO_CFG(gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
...
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB><START>
	...
	printk("%s() : level is : %d\n", __func__, level);
#endif
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB><ENDs>
	}else{
		printk("%s(): No client\n",__func__);
	}
}

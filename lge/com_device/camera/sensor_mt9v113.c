...
static int mt9v113_reg_preview(void)
{
	...
	msleep(200); //elin.lee@lge.com 2011-11-27 QM TD #10978 added for stability
...
static int mt9v113_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	...
	CDBG("%s in :%d\n",__func__, __LINE__);
	//LGE_CHANGE_S fix the WBT issue (TD#381923), jongkwon.chae@lge.com
	if (!data) {
		printk("mt9v113_sensor_init_probe failed! (data is null)\n");
		goto init_probe_fail;
	}
	//LGE_CHANGE_E fix the WBT issue (TD#381923), jongkwon.chae@lge.com
...
int mt9v113_sensor_init(const struct msm_camera_sensor_info *data)
{
	...
	if (data) {
		...
		printk("mt9v113_init failed! (data is null)\n");
		goto init_fail;
	}
	//LGE_CHANGE_E fix the WBT issue (TD#381923), jongkwon.chae@lge.com
...
{
	int rc = 0;
	printk("mt9v113_entering Sensor_release\n");

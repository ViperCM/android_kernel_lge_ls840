...
/* LGSI_ICS_UPDATE:ramesh.babu@lge.com For reference Check mach-msm/qdsp6v2/board-msm8x60-audio.c */

#define CONFIG_LGE_AUDIO_NO_NCP_MODE
#define CONFIG_LGE_AUDIO_USE_AUXMIC_FOR_SPEAKER_MODE

//LGSI_ICS_UPDATE_S:ramesh.babu@lge.com Sound Patch from GB to ICS[Start]
...
#include <mach/msm_iomap.h>
#include <asm/io.h>
//LGSI_ICS_UPDATE_E:ramesh.babu@lge.com Sound Patch from GB to ICS[End]
...
//LGSI_ICS_UPDATE_S:ramesh.babu@lge.com Sound Patch from GB to ICS[Start]
#include "timpani_profile_8x60_cayman_sprint.h"
//LGSI_ICS_UPDATE_E:ramesh.babu@lge.com Sound Patch from GB to ICS[End]
...
wmCodecCmd seq_lin_to_spkout_playback[] = 
{
    ...
    {0x02, 0x6030},
    {0x17, 0x0001}, // IN2_CLAMP 0bit 0 : clamp de-activate 1: clamp activated  
    //20111010 hyeongwoo.seo@lge.com 0x0101 -> 0x0102   // //Sound Patch from GB to ICS
    ...
    {0x22, 0x0000}, // 0dB
    {0x03, 0x0108},  
	//20111010 hyeongwoo.seo@lge.com 0x0170 ->	0x0168   //Sound Patch from GB to ICS  
    {0x25, 0x0168}, // 12dB(178}, 9dB(170}, 7.5dB(168}, 6dB(160}, 4.5dB(158}, 3dB(150}, 1.5dB(148}, 0dB(140)
	//20111010 hyeongwoo.seo@lge.com 0x013F ->	0x013c    
    {0x26, 0x013c}, // refer page 26  // 0x013e //Sound Patch from GB to ICS
    {0x17, 0x0000},  
    {0x01, 0x100B}
};
...
wmCodecCmd seq_lin_to_headset_playback[] = 
{
    ...
    {0x2F, 0x0000},
    {0x30, 0x0000},
	//Sound Patch from GB to ICS 0x0131 to 0x0137
    {0x1C, 0x0137},//{0x1C, 0x0137}, //refer to page 26 2011.7.7 dolby tuning
    {0x1D, 0x0137},//{0x1D, 0x0137}, //refer to page 26 2011.7.7 dolby tuning 
    {0x16, 0x0000},
    {0x46, 0x0100},
    {0x49, 0x0100}
};
...
wmCodecCmd seq_lin_to_headset_spkout[] = 
{
    ...
    {0x2F, 0x0000},	
    {0x30, 0x0000},
	//Sound Patch from GB to ICS 0x011D to 0x0119
    {0x1C, 0x0119}, //{0x1C, 0x0117}, //refer to page 26
    {0x1D, 0x0119}, //{0x1D, 0x0117}, //refer to page 26    
    {0x16, 0x0000},	
    {0x46, 0x0100},	
    {0x49, 0x0100}
};
...
wmCodecCmd seq_tuning_lin_to_spkout_playback[] = 
{
    ...
    {0x02, 0x6030},
    {0x17, 0x0001}, // IN2_CLAMP 0bit 0 : clamp de-activate 1: clamp activated  
//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x0101 to 0x0102
    {0x1A, 0x0102}, // IN2A -6dB(00}, -3.5dB(01}, 0dB(02}, 3.5dB(03}, 6dB(04}, 12dB(05}, 18dB(06}, 18dB(07)  
    {0x1B, 0x0102}, // IN2B -6dB(00}, -3.5dB(01}, 0dB(02}, 3.5dB(03}, 6dB(04}, 12dB(05}, 18dB(06}, 18dB(07)
    {0x36, 0x0005},
    {0x03, 0x0008},
    {0x22, 0x0000}, // 0dB
    {0x03, 0x0108},  
//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x0170 to 0x0168
    {0x25, 0x0168}, // 12dB(178}, 9dB(170}, 7.5dB(168}, 6dB(160}, 4.5dB(158}, 3dB(150}, 1.5dB(148}, 0dB(140)
//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x013F to 0x013c
...
wmCodecCmd seq_tuning_lin_to_headset_playback[] = 
{
    ...
    {0x2F, 0x0000},
    {0x30, 0x0000},
	//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x0131 to 0x0137
    {0x1C, 0x0137},//{0x1C, 0x0137}, //refer to page 26 2011.7.7 dolby tuning
    {0x1D, 0x0137},//{0x1D, 0x0137}, //refer to page 26 2011.7.7 dolby tuning 
    {0x16, 0x0000},
    {0x46, 0x0100},
...
//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x0139 to 0x013F
	{0x26, 0x013F}, //{0x26, 0x0137},//	 {0x26, 0x013F}, // refer to page 26
    {0x2D, 0x0040},
    {0x2E, 0x0010},	
    {0x03, 0x0138},
    {0x2F, 0x0000},	
    {0x30, 0x0000},	
//LGSI_ICS_UPDATE:ramesh.babu@lge.com Sound Patch from GB to ICS 0x011D to 0x0125
    {0x1C, 0x0125}, //{0x1C, 0x0117}, //refer to page 26
    {0x1D, 0x0125}, //{0x1D, 0x0117}, //refer to page 26    
    {0x16, 0x0000},	
    {0x46, 0x0100},	
    {0x49, 0x0100}
};
...
/* LGSI_ICS_UPDATE_S:ramesh.babu@lge.com Headset Pop Noise / Tick Sound issue #154026 [Start]*/
...
#endif
/* LGSI_ICS_UPDATE_E:ramesh.babu@lge.com [End] */
...
#ifdef EW0804_CHECK_WM_QTR
	printk("[EW0804]WM9093 %s \n", __func__);
#endif
...
static void msm_snddev_poweramp_off_spk(void)
{
	...
	if (lge_audio_wm9093_platform.set_amp_path)
		...
		printk("[EW0804]%s _amp_path NULL\n", __func__);
#endif
...
#endif
//LGSI_ICS_UPDATE_S:ramesh.babu@lge.com Sound Patch ICS[Start]
    {headset_ab_cpls_48KHz_osr256_actions,                6}, // HPH_RX1_L_GAIN
    {headset_ab_cpls_48KHz_osr256_actions,                7}, // HPH_RX1_R_GAIN
    {headset_ab_cpls_48KHz_osr256_actions,               25}, // HPH_L_GAIN
    {headset_ab_cpls_48KHz_osr256_actions,               26}, // HPH_R_GAIN
//LGSI_ICS_UPDATE_E:ramesh.babu@lge.com Sound Patch ICS[End]	
...
{
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
    const char* AudioSubsystemPath  = "/data/wm9093_cayman.txt";
    const char* AudioSubsystemPath2 = "/etc/wm9093_cayman.txt";
#else
...
//LGSI_ICS_UPDATE_S:ramesh.babu@lge.com WBT issue #378874, To safeguard from Segmentation fault on token value NULL [Start] 
      if (token != NULL)
      {
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
...
//LGSI_ICS_UPDATE_E:ramesh.babu@lge.com WBT issue #378874 [End]
    }
...
int get_wm9093_cal_db(char *data_array, int *data_len)
{
		...
		/* Header */
		sprintf(str, "WM9093 for %s", "LS840");
...
static int get_str_and_parameters(char *buf, char *str , long int *param1, int num_of_par)
{
	...
	token = strsep(&buf, " ");
//[LGE_START 27thjuly2012 mukesh.agarwal@lge.com @ WBT issue]
    if(token!=NULL)	
	{
//[LGE_END]	
		strcpy(str, token);
...
#if defined(CONFIG_CAST_SUPPORT)

#define WM9093_CMD_SIZE 256
typedef struct{
	char cmd_str[WM9093_CMD_SIZE];
	int cmd;
}lge_wm9093_data;

lge_wm9093_data wm9093_private;

#define	WM9093_CMD_INVALID	0
#define	WM9093_READ_REG		1
#define	WM9093_WRITE_REG	2
#define	WM9093_READ_BUF		3
#define	WM9093_WRITE_BUF	4
#define	WM9093_PATH			5
#define	WM9093_PEEK			6
#define	WM9093_POKE			7
#define	WM9093_SET_PATH		8
#define	WM9093_CMD_MAX		31

#define QTR8615_CMD_MASK	0x20
#define CHECK_QTR8615(cmd)	(cmd&QTR8615_CMD_MASK)
#define GET_QTR8615(cmd)	(cmd&0x1F)

#define	QTR8615_CMD_INVALID	0
#define	QTR8615_PATH		1
#define	QTR8615_GET_AMP		2
#define	QTR8615_PATH_INFO	3
#define	QTR8615_READ		4
#define	QTR8615_WRITE		5
#define	QTR8615_PEEK		6
#define	QTR8615_POKE		7
#define	QTR8615_CMD_MAX		31

typedef struct{
	wmCodecCmd *pCmd;
	int size;
}wmCodecList_data;
#define ADD_WM_CODEC_LIST(data)	{data,sizeof(data)/sizeof(data[0])}
wmCodecList_data wmCodecList[]=
{
	ADD_WM_CODEC_LIST(seq_power_down), //ICODEC_AMP_OFF
	{NULL,0},//ICODEC_HANDSET_RX
	ADD_WM_CODEC_LIST(seq_lin_to_headset), //ICODEC_HEADSET_ST_RX
	{NULL,0}, //ICODEC_HEADSET_MN_RX
	ADD_WM_CODEC_LIST(seq_lin_to_spkout), //ICODEC_SPEAKER_RX
	ADD_WM_CODEC_LIST(seq_lin_to_headset_spkout), //ICODEC_HEADSET_ST_RX_SPEAKER_RX
	ADD_WM_CODEC_LIST(seq_lin_to_headset_tty), //ICODEC_TTY_RX
	ADD_WM_CODEC_LIST(seq_lin_to_spkout_playback),//ICODEC_SPEAKER_PLAYBACK_RX
	ADD_WM_CODEC_LIST(seq_lin_to_headset_playback),//ICODEC_HEADSET_ST_PLAYBACK_RX		
};

static int lge_wm9093_cal_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}
...
static const qtr_codec_list lge_codec_list[]={
		{(void *)&msm_snddev_poweramp_on_spk, "[4]ICODEC_SPEAKER_RX(wm9093)"},
		{(void *)&msm_snddev_poweramp_on_spk_playback, "[7]ICODEC_SPEAKER_PLAYBACK_RX(wm9093)"},
		{(void *)&msm_snddev_poweramp_on_hph, "[2]ICODEC_HEADSET_ST_RX(wm9093)"},
		{(void *)&msm_snddev_poweramp_on_hph_playback, "[8]ICODEC_HEADSET_ST_PLAYBACK_RX(wm9093)"},			
		{(void *)&msm_snddev_poweramp_on_hph_spk, "[5]ICODEC_HEADSET_ST_RX_SPEAKER_RX(wm9093)"},
		{(void *)&msm_snddev_poweramp_on_tty, "[6]ICODEC_TTY_RX(wm9093)"},
		{(void *)&msm_snddev_enable_amic_power, "OTHC_MICBIAS_MAIN"},
		{(void *)&msm_snddev_enable_adualmic_power, "OTHC_MICBIAS_MAIN/SUB"},
		{(void *)&msm_snddev_enable_submic_power, "OTHC_MICBIAS_SUB"},
		{(void *)&msm_snddev_enable_dualmic_power, "OTHC_MICBIAS_MAIN/SUB"},
};

static void * lge_qtr8615_check_codec(int codec)
{
	int narray;
	int i;
	struct msm_snddev_info *info;
	char *array[4]={
		"speaker_stereo_rx",  //icodec
		"bt_sco_rx",		//ecodec
		"fmradio_stereo_rx", //mi2s
		"hdmi_stereo_rx", //hdmi
	};
	if(codec >=4)
		return NULL;	
	narray = msm_snddev_devcount();	
	for(i=0;i<narray;i++)
	{
		info = audio_dev_ctrl_find_dev(i);
		if(!info)
			continue;
		if(!strcmp(array[codec],info->name)){
			return info->dev_ops.open;
		}	
	}
	return NULL;
}

static int lge_qtr8615_get_opened_num(void)
{
	int narray,i;
	int cnt=0;
	...
	//printk(KERN_INFO "opened %d", cnt);
	return cnt;
}

static char *lge_qtr8615_get_codec_str(struct msm_snddev_info *info)
{
	int i=0;
	...
	if(info->dev_ops.open == lge_qtr8615_check_codec(3)){
		return  "hdmi";
	}
	if(info->dev_ops.open == lge_qtr8615_check_codec(2)){
		return  "mi2s";
	}
	if(info->dev_ops.open == lge_qtr8615_check_codec(1)){
		return  "ecodec";		
	}
	if(!info->private_data){
		return "null";
	}
	icodec = info->private_data;
	for(i=0;i<sizeof(lge_codec_list)/sizeof(lge_codec_list[0]);i++)
	{
		if(icodec->data && (lge_codec_list[i].func == (void *)icodec->data->pamp_on))
			return lge_codec_list[i].comment;
	}
	return "icodec(null)";
}
static int lge_qtr8615_read_cal(char *name, char *buff)
{
	int i=0,cnt;
	struct msm_snddev_info *info;
	struct snddev_icodec_state *icodec;
	...
	for(i=0; i < narray; i++)
	{
		...
		if(strcmp(name,info->name))
			continue;
		if(info->dev_ops.open != lge_qtr8615_check_codec(0))
			continue;
		...
		size	= pdata->profile->settings->action_sz;
		actions = pdata->profile->settings->actions;

		for(cnt=0;cnt <size;cnt++){
			sprintf(temp, "0x%08x 0x%08x ",actions[cnt].type,actions[cnt].action);
			strcat(buff,temp);
		}
		if(cnt){
...
static struct adie_codec_action_unit *lge_qtr8615_get_action(char *name,int index)
{
	int i=0;
	struct msm_snddev_info *info;
	struct snddev_icodec_state *icodec;
	...
	for(i=0; i < narray; i++)
	{
		...
		if(index >=size)
			return NULL;
		if(actions[index].type!=ADIE_CODEC_ACTION_ENTRY)
			return NULL;
...
static ssize_t lge_qtr8615_cal_read(struct file *file, char __user *buf,
			  size_t count, loff_t *ppos)
{
	char buffer[2048];
	char temp[128];
	char name[256];
	int n = 0,i;	
	lge_wm9093_data *data = file->private_data;
	int cmd=0; 
	int param[3];
	int narray;
	int opencnt=0;	
	struct adie_codec_action_unit *action;
	struct msm_snddev_info *info;

	#define PRINT_INFO(info,temp,buffer) \
		do{ \
				sprintf(temp,"%s",info->name); strcat(buffer,temp);\
				sprintf(temp," %d",info->capability); strcat(buffer,temp);\
				sprintf(temp," %d",info->copp_id); strcat(buffer,temp);\
				sprintf(temp," %d",info->acdb_id); strcat(buffer,temp);\
				sprintf(temp," %d",info->dev_volume); strcat(buffer,temp);\
				sprintf(temp," %d",info->sample_rate); strcat(buffer,temp);\
				sprintf(temp," %d",info->channel_mode); strcat(buffer,temp);\
				sprintf(temp," %d",info->set_sample_rate); strcat(buffer,temp);\
				sprintf(temp," %s",lge_qtr8615_get_codec_str(info)); strcat(buffer,temp);\
		}while(0)
	...
	if(cmd==QTR8615_PATH){
		opencnt = lge_qtr8615_get_opened_num();
		n= sprintf(buffer,"%d",opencnt);	
		if(opencnt==0)
			goto done;
		narray = msm_snddev_devcount();
		for(i=0;i<narray;i++)
		{
			info = audio_dev_ctrl_find_dev(i);
			if(info&&info->opened){
				sprintf(temp," "); strcat(buffer,temp);
				PRINT_INFO(info,temp,buffer);			
 			}
		}
		n= strlen(buffer);		
	}
	else if(cmd==QTR8615_GET_AMP){
		sscanf(data->cmd_str, "%s",temp);		
		narray = msm_snddev_devcount();
		for(i=0;i<narray;i++)
		{
			info = audio_dev_ctrl_find_dev(i);
			if(info&& !strcmp(temp,info->name)){
 				n=sprintf(buffer,"%s",lge_qtr8615_get_codec_str(info));
				break;
			}
		}
		n= strlen(buffer);	
	}
	else if(cmd==QTR8615_PATH_INFO){
		sscanf(data->cmd_str, "%s",name);		
		narray = msm_snddev_devcount();
		for(i=0;i<narray;i++)
		{
			info = audio_dev_ctrl_find_dev(i);
			if(info&&!strcmp(name,info->name)){
				PRINT_INFO(info,temp,buffer);
				break;
 			}
		}
	...
	else if(cmd==QTR8615_READ){
		sscanf(data->cmd_str, "%s",name);
		lge_qtr8615_read_cal(name,buffer);
		n= strlen(buffer);			
	}
	else if(cmd==QTR8615_PEEK){
		sscanf(data->cmd_str, "%s %d",name,&param[0]);
		action = lge_qtr8615_get_action(name,param[0]);
		if(action){		
			sprintf(buffer, "0x%08x,0x%08x",action->type,action->action);
			n= strlen(buffer);			
		}
	}
	else if(cmd==QTR8615_POKE){
		sscanf(data->cmd_str, "%s %d %d",name,&param[0],&param[1]);
		action = lge_qtr8615_get_action(name,param[0]);
		if(action){		
			action->action = (u32)param[1];
			sprintf(buffer, "0x%08x 0x%08x",action->type,action->action);
			n= strlen(buffer);			
		}		
	}
	...
	buffer[n] = 0;
	return simple_read_from_buffer(buf, count, ppos, buffer, n+1);
}

static ssize_t lge_wm9093_cal_read(struct file *file, char __user *buf,
			  size_t count, loff_t *ppos)
{
	char buffer[2048];
	char temp[128];
	...
	if(CHECK_QTR8615(cmd)){
		return lge_qtr8615_cal_read(file,buf,count,ppos);
	}
	
	if(cmd==WM9093_READ_REG){
		sscanf(data->cmd_str, "%d", &param[0]);
		wmCmd.wmaddress = param[0];
		lge_audio_wm9093_platform.wm9093_read_rgister(&wmCmd);
		n= sprintf(buffer,"0x%x",wmCmd.wmdata);
	}
	else if(cmd==WM9093_WRITE_REG){
		sscanf(data->cmd_str, "%d,%d", &param[0],&param[1]);
		wmCmd.wmaddress = param[0];
		wmCmd.wmdata = param[1];
		lge_audio_wm9093_platform.wm9093_cmd_register(wmCmd);
		lge_audio_wm9093_platform.wm9093_read_rgister(&wmCmd);
		n= sprintf(buffer,"0x%x",wmCmd.wmdata);
	}
	else if(cmd==WM9093_READ_BUF){
		narray = sizeof(wmCodecList)/sizeof(wmCodecList[0]);
		sscanf(data->cmd_str, "%d", &param[0]);
		if(param[0]>=narray || wmCodecList[param[0]].pCmd==NULL){
			n= sprintf(buffer,"%d,%d",narray,0); //icodec, array size
		}
		else{
			sprintf(buffer,"%d,%d",param[0],wmCodecList[param[0]].size);
			for(i=0;i< wmCodecList[param[0]].size;i++){
					sprintf(temp,",%d,%d",wmCodecList[param[0]].pCmd[i].wmaddress,wmCodecList[param[0]].pCmd[i].wmdata);
					strcat(buffer,temp);
			}	
		}
		n = strlen(buffer);	
	}
	else if(cmd==WM9093_WRITE_BUF){

	}
	else if(cmd==WM9093_PATH){
		n= sprintf(buffer,"0x%x",lge_audio_wm9093_platform.icodec_num);		
	}
	else if(cmd==WM9093_PEEK){
		narray = sizeof(wmCodecList)/sizeof(wmCodecList[0]);
		sscanf(data->cmd_str, "%d,%d", &param[0],&param[1]);
		if(param[0]>=narray || wmCodecList[param[0]].pCmd==NULL){
			n= sprintf(buffer,"%d,%d,%d",narray,0,0);
		}
		else{
			found = 0;
			for(i=0;i< wmCodecList[param[0]].size;i++){
				if(wmCodecList[param[0]].pCmd[i].wmaddress == param[1]){
					n= sprintf(buffer,"%d,%d,%d",param[0],param[1],wmCodecList[param[0]].pCmd[i].wmdata);
					found =1;
					break;
				}
			}
	...
	else if(cmd==WM9093_POKE){
		narray = sizeof(wmCodecList)/sizeof(wmCodecList[0]);
		sscanf(data->cmd_str, "%d,%d,%d", &param[0],&param[1],&param[2]);
		if(param[0]>=narray || wmCodecList[param[0]].pCmd==NULL){
			n= sprintf(buffer,"%d,%d,%d",narray,0,0);
		}
		else{
			...
			for(i=0;i< wmCodecList[param[0]].size;i++){
				if(wmCodecList[param[0]].pCmd[i].wmaddress == param[1]){
					wmCodecList[param[0]].pCmd[i].wmdata = param[2];
					n= sprintf(buffer,"%d,%d,%d",param[0],param[1],param[2]);
					found =1;
					break;
				}
			}
	...
	else if(cmd ==WM9093_SET_PATH){
		sscanf(data->cmd_str, "%d", &param[0]);
		if(param[0] <= ICODEC_HEADSET_ST_PLAYBACK_RX){
			printk(KERN_INFO "WM9093 Set Path = %d", param[0]);
			lge_audio_wm9093_platform.set_amp_path(param[0]);
			n= sprintf(buffer,"%d",param[0]);
		}
		else{
			printk(KERN_ERR "WM9093 Set Path Failure %d",param[0]);
		}
	}
	buffer[n] = 0;
	return simple_read_from_buffer(buf, count, ppos, buffer, n+1);
...
static ssize_t lge_wm9093_cal_write(struct file *filp,
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
		lge_wm9093_data *wm9093_data = filp->private_data;
		char cmd_str[WM9093_CMD_SIZE];
		char *token;
		int cmd;
		int rc;
		...
		if (cnt > WM9093_CMD_SIZE - 1)
			return -EINVAL;
	
		rc = copy_from_user(cmd_str, ubuf, cnt);
		if (rc)
			return -EFAULT;
		...
		token = cmd_str;
		while(*token&&(*token++!=']'));
		strcpy(wm9093_data->cmd_str,token);
		wm9093_data->cmd= cmd;
...
static const struct file_operations wm9093_cal_fops = {
	.open = lge_wm9093_cal_open,	
	.read = lge_wm9093_cal_read,
	.write= lge_wm9093_cal_write,
};

#endif /*CONFIG_CAST_SUPPORT*/
...
#if defined(CONFIG_CAST_SUPPORT)
		debugfs_dump = debugfs_create_file("cast",
		S_IFREG | S_IRUGO, debugfs_wm9093_cal_tool_config,
		(void *)&wm9093_private, &wm9093_cal_fops);
#endif	
	}

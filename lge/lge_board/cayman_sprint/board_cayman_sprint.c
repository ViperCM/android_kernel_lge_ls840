...
//[LS840][ICS][Display]<kurva.venumadhav@lge.com>
...
#endif
//LG_UPDATE_S: [sinjo.mattappallil@lge.com] [11 June 2012] [Porting from LS840 GB]
#ifdef CONFIG_LGE_DIAGTEST
// MOD 0009214: [DIAG] LG Diag feature added in side of android
...
#endif 
static struct platform_device testmode_sysfs_device = {
	.name = "testmode_sysfs",
	.id = -1,
};
...
//LG_UPDATE_E: [sinjo.mattappallil@lge.com] [11 June 2012] [Porting from LS840 GB]
...
int get_usb_lock(void)
{
	...
	int state;
	state = lg_get_usb_lock_state();
	printk("hyoill.leem recevied usb lock state = %d\n",state);
	if (state)
		user_diag_enable = 1;
	else
		user_diag_enable = 0;
...
void set_usb_lock(int lock)
{
	...
	printk("hyoill.leem send to modem rapi diag lock : %d\n",lock);
...
//Add NFC Porting ,addy.kim@lge.com, 2012-05-21 [START]
#if defined (CONFIG_LGE_NFC_NXP_PN544PN65N)
static struct msm_i2c_platform_data msm_gsbi11_qup_i2c_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
...
#endif
//Add NFC Porting ,addy.kim@lge.com, 2012-05-21 [END]
//LGSI_ICS_UPDATE_S::<krishnakanth.satpadi@lge.com><2012-JUN-06><Enabled sensor based on LS840 GB><START>
#if 0
static struct msm_i2c_platform_data msm_gsbi12_qup_i2c_pdata = {
	.clk_freq = 100000,
...
#define IPRJ_ION_MEM_SETTING //[LS840-ICS][1080p videoplayback]kurva.venumadhav@lge.com
...
#define MSM_ION_WB_SIZE		0x2100000 /* 33MB */ //[LS840-ICS][1080p videoplayback]kurva.venumadhav@lge.com
#else
...
#define MSM_ION_HEAP_NUM	8   //kurva.venumadhav@lge.com ported from vs840-ics
...
/* LGSI_ICS_UPDATE_S:ramesh.babu@lge.com Kernel Panic Handler [Start] */
...
#endif
/* LGSI_ICS_UPDATE_E:ramesh.babu@lge.com kernel Panic Handler [End] */
...
int emergency_dload_lock;

static int __init emergency_dload_lock_setup(char *emergency_dload_lock_str)
{

    if(!strcmp(emergency_dload_lock_str, "1"))
        emergency_dload_lock = 1;
    else if(!strcmp(emergency_dload_lock_str, "0"))
        emergency_dload_lock = 0;
    else
        emergency_dload_lock = 2;

        return 1;
}
__setup("lge.emergency_dload_lock=", emergency_dload_lock_setup);
...
#if 0		//kurva.venumadhav@lge.com ported from VS840-ICS
		{
...
//	msm8x60_reserve_table[MEMTYPE_EBI1].size += MSM_ION_CAMERA_SIZE; //kurva.venumadhav@lge.com ported from vs840-ics
...
static int __init battery_information_setup(char *batt_info)
{
    ...
    else if(!strcmp(batt_info, "isl6296"))
    {
            ...
            msm_charger_data.min_voltage = 3400;
	//FIXME 3160_conflict		
...
//LG_UPDATE_S: [mukesh.agarwal@lge.com] [21 June 2012] [LED control]
...
//LG_UPDATE_E: [mukesh.agarwal@lge.com] [21 June 2012] [LED control]
};
#else
static struct pmic8058_led pmic8058_flash_leds[] = {
	[0] = {
...
//BEGIN : [LS840_ICS][SPR][H/W Requirement add]  sanghyuk.nam@lge.com 2012.08.23
//	gpio_tlmm_config(GPIO_CFG(CONFIG_BCMDHD_GPIO_WL_HOSTWAKEUP, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(CONFIG_BCMDHD_GPIO_WL_HOSTWAKEUP, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
//END : [LS840_ICS][SPR][H/W Requirement add]  sanghyuk.nam@lge.com 2012.08.23

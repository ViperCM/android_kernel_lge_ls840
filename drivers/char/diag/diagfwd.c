...
// LGE_CHANGE_S [jaekyung83.lee@lge.com] [2011.10.21] [CAYMAN] enable or disable key logging status of slate [START]
#if defined (CONFIG_MACH_LGE_C1_BOARD_SPR)
#include "../../../lge/include/lge_diag_icd.h"
#endif
// LGE_CHANGE_E [jaekyung83.lee@lge.com] [2011.10.21] [CAYMAN] enable or disable key logging status of slate [END]
...
#ifdef CONFIG_LGE_USB_ACCESS_LOCK
		if (user_diag_enable == 1 && ((proc_num == APPS_DATA)))
			return 0;
#endif
		if (proc_num == APPS_DATA) {
...
#if defined (CONFIG_LGE_USB_DIAG_DISABLE) && !defined(CONFIG_LGE_USB_ACCESS_LOCK)
int user_diag_func_enable =1;
int get_user_diag_enable(void)
{
        return user_diag_func_enable;
}
EXPORT_SYMBOL(get_user_diag_enable);
#endif
/* 2011.12.07 jaeho.cho@lge.com diag enable/disable*/
...
#ifdef CONFIG_LGE_USB_ACCESS_LOCK
	// LGE_CHANGE_S moses.son@lge.com
	if (buf[0]!=0xA1 && user_diag_enable == 1)
		return 0;
#endif

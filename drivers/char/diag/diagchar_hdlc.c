...
// enable to send more than maximum packet size limitation
#if defined (CONFIG_MACH_LGE_C1_BOARD_SPR)
int diag_hdlc_encode_slate(struct diag_send_desc_type *src_desc,
...

{
	...
	if (src_desc && enc_dest) {
		...
		{
			printk(KERN_ERR " dest or dest_last may be NULL to trigger a   state transition only, LINE:%s,%d \n",__func__,__LINE__);
			return -1;
		}
		/* Copy local variables back into the encode structure. */
...
int lg_diag_write_overflow(void *pStart, void *pDone, void *pEnd)
{
  ...
  {
	printk(KERN_ERR "%s() diagmem_alloc failed, LINE:%d\n",__func__, __LINE__);
	return err;
  }

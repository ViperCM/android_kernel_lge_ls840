...
#define AUTO_CHARGING_VBATDET				4320
#define AUTO_CHARGING_VBATDET_DEBOUNCE_TIME_MS		3000
#define AUTO_CHARGING_VEOC_VBATDET			4270
#else
...
#define AUTO_CHARGING_RESUME_MV_CALC(x) (AUTO_CHARGING_RESUME_MV-(x*150))
#else
#define AUTO_CHARGING_RESUME_MV				4100
#endif
#ifdef CONFIG_LGE_CHARGER_VOLTAGE_CURRENT_SCENARIO
#define AUTO_CHARGING_VMAXSEL_CALC(x) (AUTO_CHARGING_VMAXSEL-(x*150))
#define AUTO_CHARGING_VBATDET_CALC(x) (AUTO_CHARGING_VBATDET-(x*150))
#define AUTO_CHARGING_VEOC_VBATDET_CALC(x) (AUTO_CHARGING_VEOC_VBATDET-(x*150))
#define AUTO_CHARGING_RESUME_MV_CALC(x) (AUTO_CHARGING_RESUME_MV-(x*150))
#endif
...
#define DEFAULT_THRESHOLD_LOWER_CALC(x)		(DEFAULT_THRESHOLD_LOWER - (x*100))
#define DEFAULT_THRESHOLD_UPPER_CALC(x)		(AUTO_CHARGING_VMAXSEL - (x*150))
...
static int resume_mv = AUTO_CHARGING_RESUME_MV-150;
...
static int resume_mv_set(const char *val, struct kernel_param *kp)
{
	...
	if (is_chg_plugged_in()) {
#endif
#if defined (CONFIG_LGE_CHARGER_VOLTAGE_CURRENT_SCENARIO)
	rc = pm8xxx_batt_alarm_threshold_set(
			PM8XXX_BATT_ALARM_LOWER_COMPARATOR, AUTO_CHARGING_RESUME_MV_CALC(lge_battery_info));
	if (!rc)
		rc = pm8xxx_batt_alarm_threshold_set(
			PM8XXX_BATT_ALARM_UPPER_COMPARATOR, AUTO_CHARGING_VMAXSEL_CALC(lge_battery_info));
#else
	rc = pm8xxx_batt_alarm_threshold_set(
...
static void pm8058_chg_enable_irq(int interrupt)
{
	if (!__test_and_set_bit(interrupt, pm8058_chg.enabled_irqs)) {
		...
		printk(KERN_ERR "===%s: auto charging done IRQ enable!: %d ===\n",__func__,interrupt);
	}
}
...
static void pm8058_chg_disable_irq(int interrupt)
{
	if (__test_and_clear_bit(interrupt, pm8058_chg.enabled_irqs)) {
		...
		printk(KERN_ERR "===%s: auto charging done IRQ disable!: %d ===\n",__func__,interrupt);
	}
}
...
int testmode_charging_onoff = 1;
EXPORT_SYMBOL(testmode_charging_onoff);
#endif
...
{
	dev_info(pm8058_chg.dev, "%s chg_done_check_work"
		"\n", __func__);
	chg_done_cleanup();
}
...
static void charging_check_work(struct work_struct *work)
{
	...
	dev_info(pm8058_chg.dev, "%s charging_check_work: state is %d"
		"\n", __func__,fsm_state);
	switch (fsm_state) {
...
// for charging scenario.
	 ...
	 * delay.
	 */
	printk("%s: charging_check_work delay work call~ \n", __func__);	 
...
int pm8058_start_charging_for_TESTMODE(void)
{
  ...
  chg_current = 800;
#if defined(CONFIG_MACH_LGE_C1_BOARD_SPR)  
	pm8058_chg.disabled = 0;

	testmode_charging_onoff = 1;
#endif
	pm8058_chg.current_charger_current = chg_current;
	pm8058_chg_enable_irq(FASTCHG_IRQ);
...
#if defined(CONFIG_MACH_LGE_C1_BOARD_SPR)
static int pm8058_get_xo_therm_adc(void)
{
	int mv_reading;

	mv_reading = 0;
	batt_read_adc(CHANNEL_ADC_XOTHERM, &mv_reading);
	pr_err("%s: xo_therm_raw is %d\n", __func__, mv_reading);
	return mv_reading;
}

static int pm8058_get_acc_adc(void)
{
	int mv_reading;

	mv_reading = 0;
	batt_read_adc(CHANNEL_ADC_ACC, &mv_reading);
	pr_err("%s: acc_adc_raw is %d\n", __func__, mv_reading);
	return mv_reading;
}

int testmode_read_xo_therm_adc(void)
{
	int xo_adc;
	xo_adc = pm8058_get_xo_therm_adc();
	return xo_adc;
}
EXPORT_SYMBOL(testmode_read_xo_therm_adc);

int testmode_read_acc_adc(void)
{
	int acc_adc;
	acc_adc = pm8058_get_acc_adc();
	return acc_adc;
}
EXPORT_SYMBOL(testmode_read_acc_adc);
#endif
	...
	case 1:
		...
		if (is_chg_plugged_in()) {
			...
			if (rc)
				...
				msm_charger_notify_event(&usb_hw_chg, CHG_BATT_NEEDS_RECHARGING);
		}
#else
		rc = pm8xxx_batt_alarm_disable(
				PM8XXX_BATT_ALARM_UPPER_COMPARATOR);

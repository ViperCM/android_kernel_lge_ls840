...
struct android_dev {
    ...
    bool pif_connected; //for initial value
#endif
...
void android_diag_work(struct work_struct *data) {
...
void diag_lock_work(void)
{
	struct android_dev		*dev = _android_dev;
	printk("@@@@@@@@@@@@ hyoill.leem %s\n",__func__);
	schedule_delayed_work(&dev->diag_work,msecs_to_jiffies(30000));
}
EXPORT_SYMBOL(diag_lock_work);
					...
					struct usb_configuration *c)
{
	...
	char buf[32], *b;
	int err = -1, i;
	static int acm_initialized = 0, ports = 0;
...
{
        snprintf(snum, MAX_SERIAL_LEN, serial_string);
}
...
static int __init init(void)
{
	...
	INIT_DELAYED_WORK(&dev->diag_work, android_diag_work);
